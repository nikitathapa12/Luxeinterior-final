...
const CartPage = () => {
  ...
  const getToken = async () => {
    try {
      const { data } = await axios.get("http://localhost:3000/api/products/braintree/token");
  ...
  const handlePayment = async () => {
    try {
      ...
      const { data } = await axios.post("http://localhost:3000/api/products/braintree/payment", {
        nonce,
        cart,
      });
      setLoading(false);
        ...
        <div className="flex flex-wrap ml-10 mt-5">
          <div className="w-full md:w-3/4">
            <table className="min-w-full bg-white border">
              <thead>
                <tr className="w-full bg-blue-950 text-white border-b">
